...
export class UsersService {
	...
	async create(createUserDto: CreateUserDto): Promise<User> {
		...
		const userExist = await this.findOneOnRepository(findUserByEmailQuery(email));
	...
	async remove(userId: string) {
		...
		const profileRemoved = await this.profilesService.remove(profile.profile_id);

		const passwordRemoved = await this.passwordsService.remove(password.password_id);
		return {
			...userResultRemoved,
			user_id,
			email,
